...
import { useGetOnTheAirTvSeriesQuery } from "../redux/services/tvSeriesApi";
...
const TvOnTheAir = () => {
  ...
  const pageNum = useRef(tv.onTheAri);

  const { activeGenreIds } = useSelector((state) => state.genreSlice);

  const { data, isSuccess, isLoading, status } = useGetOnTheAirTvSeriesQuery(
    pageNum.current
  );
